"""
Cleanup Page: Progress indicator during cleanup, and completion celebration report.
"""
...
class CleanupPage(QWidget):
    """Cleanup execution screen and final recovery report."""
    ...
    def _init_ui(self) -> None:
        ...
        # Done button (shown upon completion)
        self.btn_done = QPushButton("  Return to Dashboard  ")
        ...
        self.main_layout.addWidget(self.btn_done, alignment=Qt.AlignCenter)
    ...
    def update_progress(self, processed: int, total: int, recovered: int, cur_path: str) -> None:
        ...
        self.lbl_current_path.setText(f"Processing: {display_path}")

    def show_completion(self, summary: CleanupSummary) -> None:
        """Display successful completion metrics."""
        self.prog_bar.setValue(100)
        self.lbl_status.setText("Cleanup Completed Successfully!")
        ...
        self.lbl_recovered.setText(f"🎉 {recovered_str} Cleaned and Reclaimed!")
        self.lbl_recovered.setStyleSheet("font-size: 22px; font-weight: 800; color: #10B981;")

        self.lbl_current_path.setText(
            f"Removed {format_number(summary.files_deleted)} files. "
            f"{summary.files_skipped} skipped (in-use or locked). "
            f"0 personal or system files touched."
        )
